...
import { isTintRemote } from './deviceCategory';
...
export default function SwitchesTab({ sendToAdapter, t, alive }) {
	...
	const rows = sensors
		? Object.entries(sensors).filter(([, s]) => s.type && s.type.includes('Switch') && !isTintRemote(s))
		: [];
					...
					<Table size="small">
						<TableHead sx={tableHeadSx}>
							<TableRow>
								...
								<TableCell align="right"><strong>{t('colLastEvent')}</strong></TableCell>
							</TableRow>
						</TableHead>
						<TableBody>
							{rows.map(([id, sensor]) => {
								...
								return (
									<TableRow key={id} sx={tableRowSx}>
										...
										<TableCell>
											<Typography variant="body2">{sensor.modelid || '–'}</Typography>
										</TableCell>
										<TableCell>
											<Typography variant="body2">{sensor.manufacturername || '–'}</Typography>
										</TableCell>
										...
										<TableCell align="right">
											<Typography variant="body2">
												{s.buttonevent !== undefined ? s.buttonevent : '–'}
											</Typography>
										</TableCell>
									</TableRow>
								);
