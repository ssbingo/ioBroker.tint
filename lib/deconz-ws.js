...
class DeconzWebSocket {
	...
	constructor(options) {
		...
		this.log.debug(
			`DeconzWebSocket initialised — endpoint: ws://${this.ip}:${this.wsPort}`,
		);
	}
	...
	connect() {
		...
		this._ws.on('message', data => {
			...
			this.log.debug(
				`WebSocket ← e="${event.e}" r="${event.r}" id="${event.id}"` +
				(event.t ? ` t="${event.t}"` : '') +
	...
	_scheduleReconnect() {
		...
		this.log.debug(
			`WebSocket: back-off updated ${this._reconnectDelay / 1000}s → ${nextDelay / 1000}s`,
		);
