...
class DeconzWebSocket {
	...
	close() {
		...
		if (this._reconnectTimer) {
			this.log.debug('WebSocket: cancelling pending reconnect timer');
			// Timer was created via the adapter-tracked setTimeout — adapter-core clears
			// it automatically on unload. Setting _stopped = true above already ensures
			// the callback is a no-op if it fires before then.
			this._reconnectTimer = null;
		}
		if (this._ws) {
