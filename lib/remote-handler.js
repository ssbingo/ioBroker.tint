...
class RemoteHandler {
	...
	async handleEvent(sensorId, state, config, attr) {
		...
		if (groupStr !== null) {
			...
			this.adapter.log.debug(
				`Remote ${sensorId}: zone=${zone} (config.group="${groupStr}")`,
			);
	...
	async _handleColorWheel(sensorId, prefix, state) {
		...
		this.adapter.log.debug(
			`Remote ${sensorId}: color wheel — angle=${angle}°, ` +
			`xy=[${x.toFixed(4)}, ${y.toFixed(4)}], hex=${hex}`,
		);
	...
	async _handleColorTemp(prefix, state, pressType) {
		...
		this.adapter.log.debug(
			`${prefix}: color temperature — ${mired} mired = ${kelvin} K (${pressType})`,
		);
